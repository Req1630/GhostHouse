...
bool CStage::LoadMap()
{
	if( m_pMap.size() == 0 ){
		...
		if (m_bOneCall == true) {
			for (size_t i = 0; i <= m_pMapList.size(); i++) {
				m_OldTextNum.emplace_back();
				m_bOneCallInit.emplace_back(true);
			}
			m_bOneCall = false;
		}
		...
		if (m_bOneCallInit[init_Map] == true) {
			InitTextNum(init_Map);
		}
...
//			m_NowMapScene = MapScene::In_the_house_F1_end;
	...
	if (m_bOneCallInit[nextMap] == true) {
		InitTextNum(nextMap);
	}
...
void CStage::InitTextNum(int Num) {
	...
	{
	case 0:
		m_OldTextNum[0] = m_TextNum;
		m_bOneCallInit[0] = false;
		break;
	case 1:
		m_OldTextNum[1] = m_TextNum;
		m_bOneCallInit[1] = false;
		break;
	case 2:
		m_OldTextNum[2] = m_TextNum;
		m_bOneCallInit[2] = false;
		break;
	case 3:
		m_OldTextNum[3] = m_TextNum;
		m_bOneCallInit[3] = false;
		break;
	case 4:
		m_OldTextNum[4] = m_TextNum;
		m_bOneCallInit[4] = false;
		break;
	case 5:
		m_OldTextNum[5] = m_TextNum;
		m_bOneCallInit[5] = false;
		break;
	case 6:
		m_OldTextNum[6] = m_TextNum;
		m_bOneCallInit[6] = false;
		break;
	default:
		break;
	}
