...
CSpriteUI::CSpriteUI( ID3D11Device* pDevice11,
...
void CSpriteUI::Render()
{
	...
	D3DXMatrixTranslation(
		&mTran,	//(out).
		m_SState.vPos.x, m_SState.vPos.y, m_SState.vPos.z );	// x, y, z座標.
...
void CSpriteUI::Render(float UV_x, float UV_y)
{
	...
	if (SUCCEEDED(
		...
		m_UV.x = (m_SState.Stride.w / m_SState.Base.w) * UV_x;
		m_UV.y = (m_SState.Stride.h / m_SState.Base.h) * UV_y;;

		cb.vUV = D3DXVECTOR2(m_UV.x, m_UV.y);
...
//深度(Z)ﾃｽﾄON/OFF切り替え.
void CSpriteUI::SetDepth( bool flag )
{
