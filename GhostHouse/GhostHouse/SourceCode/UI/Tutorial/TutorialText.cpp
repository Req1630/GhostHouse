...
void CTutorialText::TextDraw( int ArrayNumber, shared_ptr<CObjectBase> pObj )
{
	...
	// その場合一つ値を減らす.
	if( CTutorialTextLoad::GetTextArraySize() == ArrayNumber ){
		ArrayNumber = CTutorialTextLoad::GetTextArraySize() - 1;
	}
	int xx = 0;
	...
	if( m_TextNumber < CTutorialTextLoad::GetTextTwoArraySize( ArrayNumber ) ){
